# Script to convert COCO annotation into Labelme individual images and jsons
# Adjusted from https://gist.github.com/travishsu/6efa5c9fb92ece37b4748036026342f6
import os
import json
...
class CocoDatasetHandler:
    ...
    def rle2shape(self, row):
        ...
        shapes = [
            [[int(point[1]), int(point[0])] for point in polygon] for polygon in points
        ]
        return shapes
    ...
    def save_labelme(self, file_names, dirpath, save_json_only=False):
        ...
        for file in file_names:
            ...
            if not save_json_only:
                ...
                # copy(os.path.join(self.imgpath, file), png_out)
                copy(os.path.join(self.imgpath, file), dirpath)


# coco_json_path = (
#     "HIFIVE_Data_train_t01_val_t05_Split_20230830-1415/val/PNGImages-annotations.json"
# )
coco_json_path = "H:\Datasets\HiFive-14.4\Dataset-Cleaned-Volumes-coco-annotations.json"

img_path = "H:\Datasets\HiFive-14.4\Enhanced"

output_folder = "HIFIVE_WP14.4-Test"

dataset = CocoDatasetHandler(coco_json_path, img_path)
dataset.coco2labelme()
dataset.save_labelme(dataset.labelme.keys(), output_folder)
